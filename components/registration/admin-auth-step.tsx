...
export function AdminAuthStep({ onSuccess }: AdminAuthStepProps) {
    ...
    const handleSubmit = async (formData: FormData) => {
        ...
        try {
            ...
            // Step 2: Check if user is immediately signed in (email confirmation disabled)
            const { data: { user } } = await supabase.auth.getUser();

            if (user) {
                // Step 3: Automatically create admin profile
                await createAdminProfile(user.id, email, namaSekolah);
                // Force a page refresh to update navigation
                window.location.reload();
            } else {
                // If email confirmation is required, we'll create the profile after confirmation
                setMessage({
                    success: "Silakan periksa email Anda untuk mengkonfirmasi pendaftaran. Setelah konfirmasi, profil admin akan dibuat otomatis.",
                });
            }
        } catch (error: any) {
    ...
    const createAdminProfile = async (userId: string, email: string, namaSekolah: string) => {
        try {
            const profileData = {
                id: userId,
    ...
    const handleGoogleAuth = async () => {
        ...
        try {
            ...
            const namaSekolah = document.querySelector<HTMLInputElement>('input[name="namaSekolahGoogle"]')?.value;

            if (!namaSekolah) {
                setMessage({ error: "Nama sekolah wajib diisi untuk registrasi dengan Google" });
                setIsLoading(false);
                return;
            }

            // Store school name temporarily for use after OAuth callback
            localStorage.setItem('admin_school_name', namaSekolah);
            localStorage.setItem('admin_registration', 'true');
        ...
        const checkAuth = async () => {
            ...
            if (user) {
                ...
                if (isAdminRegistration && schoolName) {
                    try {
                        await createAdminProfile(user.id, user.email || '', schoolName);
                        localStorage.removeItem('admin_school_name');
                        localStorage.removeItem('admin_registration');
                    } catch (error) {
